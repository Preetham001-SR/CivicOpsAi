...
class BaseAgent(ABC, Generic[InputT, OutputT]):
    ...
    def _serialize_for_json(self, obj: Any) -> Any:
        ...
        if isinstance(obj, enum.Enum):
            # Convert enum to lowercase for database enum compatibility
    ...
    async def _log_execution(
        ...
        try:
            input_dict = self._serialize_for_json(input_data) if hasattr(input_data, 'model_dump') else {"data": str(input_data)}
            output_dict = self._serialize_for_json(result.output) if result.output and hasattr(result.output, 'model_dump') else {"data": str(result.output) if result.output else None}
            
            # Explicitly convert enum to its value for database compatibility
            agent_type_value = self.agent_type.value if isinstance(self.agent_type, enum.Enum) else str(self.agent_type).lower()
