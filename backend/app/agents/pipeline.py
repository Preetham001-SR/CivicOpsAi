...
class ReviewTier(str, enum.Enum):
    ...
    OPTIONAL_REVIEW = "optional_review"  # 70-90% confidence
    MANDATORY_REVIEW = "mandatory_review"  # < 70% confidence
...
class CivicOpsPipeline:
    ...
    async def _intake_node(self, state: ComplaintState) -> ComplaintState:
        with tracer.start_as_current_span("pipeline.intake") as span:
            ...
            async with AsyncSessionLocal() as db:
                ...
                if result.output:
                    state["photo_url"] = result.output.photo_url or state.get("photo_url")
                    state["audio_url"] = result.output.audio_url or state.get("audio_url")
                    if result.output.validation_errors:
                        state["errors"].extend(result.output.validation_errors)
            ...
            async with AsyncSessionLocal() as db:
                ...
                if result.success and result.output:
                    ...
                    if result.metadata.get("skipped"):
                        logger.info("vision_skipped", complaint_id=str(state["complaint_id"]), reason=result.metadata.get("reason"))
                else:
        ...
        with tracer.start_as_current_span("pipeline.speech") as span:
            ...
            async with AsyncSessionLocal() as db:
                ...
                if result.success and result.output:
                    ...
                    if result.metadata.get("skipped"):
                        logger.info("speech_skipped", complaint_id=str(state["complaint_id"]), reason=result.metadata.get("reason"))
                else:
    ...
    def _determine_review_tier(self, confidence_score: Optional[float]) -> ReviewTier:
        """Determine review tier based on confidence score."""
        if confidence_score is None:
            return ReviewTier.MANDATORY_REVIEW
        if confidence_score >= settings.CONFIDENCE_THRESHOLD_AUTO:
            return ReviewTier.AUTO
        elif confidence_score >= settings.CONFIDENCE_THRESHOLD_REVIEW:
            return ReviewTier.OPTIONAL_REVIEW
        else:
            return ReviewTier.MANDATORY_REVIEW
    
    def _should_human_review(self, state: ComplaintState) -> str:
        """Route to appropriate next step based on confidence score."""
        confidence = state.get("confidence_score", 0.0)
        review_tier = self._determine_review_tier(confidence)
        
        state["review_tier"] = review_tier.value
        
        logger.info(
            "review_tier_determined",
            complaint_id=str(state["complaint_id"]),
            confidence=confidence,
            review_tier=review_tier.value,
        )
        
        if review_tier == ReviewTier.MANDATORY_REVIEW:
            return "human_review"
        elif review_tier == ReviewTier.OPTIONAL_REVIEW:
            return "human_review"  # Optional review still goes to human review queue
        else:
            return "work_order"
    ...
    async def _human_review_node(self, state: ComplaintState) -> ComplaintState:
        with tracer.start_as_current_span("pipeline.human_review") as span:
            ...
            review_tier = state.get("review_tier", "mandatory_review")
            ...
            if review_tier == "optional_review":
                state["human_review_notes"] = "Auto-approved (optional review tier, no human action taken within timeout)"
            else:
